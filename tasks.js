 ...
 * vis-1 widget set (`widgets/fancyswitch.html` and `widgets/fancyswitch/**`), which is maintained by hand and
...
const TARGET = 'widgets/vis-2-widgets-fancyswitch';
...
function syncLegacyVersion() {
    ...
    const content = fs.readFileSync(`${__dirname}/widgets/fancyswitch.html`, 'utf8');
    const updated = content
        .replace(/version: "\d+\.\d+\.\d+"/g, `version: "${pack.version}"`)
        .replace(/version: "\d+\.\d+\.\d+",/g, `version: "${pack.version}",`);
    if (content !== updated) {
        fs.writeFileSync(`${__dirname}/widgets/fancyswitch.html`, updated);
        console.log('widgets/fancyswitch.html updated');
    }
}
