 ...
 * `widgets/fancyswitch.html` - vis-2 replaces a widget type when a React widget declares the same id. And
 * because the widget data of a project is stored per attribute name, every attribute the vis-1 template offered
 * has to exist here as well, or a migrated widget silently loses that setting.
 *
...
const PREV_PREFIX = 'widgets/vis-2-widgets-fancyswitch/';
const PUBLIC = path.join(HERE, 'public');
const LEGACY_HTML = path.join(HERE, '..', 'widgets', 'fancyswitch.html');
...
/**
 * Attributes the vis-1 template offered but that the React version does not. Existing values stay in the
 * project untouched, they simply have no effect - add to this list only after confirming the vis-1 code really
 * ignored the attribute, or that it cannot mean anything any more.
 */
const DROPPED = {
    // `duration` was the jQuery animation of the handle and is still there; `test` too. Nothing is dropped
    // from the iButton.
};
...
for (const name of WIDGETS) {
    ...
    if (info.visSet !== 'fancyswitch') {
        console.log(`ERROR ${prefix}: visSet is "${info.visSet}", must be "fancyswitch"`);
        problems++;
    }
    ...
    if (!info.visPrev?.startsWith(PREV_PREFIX)) {
        ...
        console.log(`ERROR ${prefix}: the preview ${info.visPrev} is not in public/ - run "npm run assets"`);
        problems++;
    }
